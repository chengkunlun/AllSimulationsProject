...
-(void)initializePlayer:(CGRect)frame
{
    ...
    self.playerHudBottom.frame = CGRectMake(0, 0, frameWidth, 30);
    ...
    //Play Pause Button暂停按钮
    self.playPauseButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    self.playPauseButton.frame = CGRectMake(0, 0, 25, 25);
    ...
    //Seek Time Progress Bar进度条
    self.progressBar = [[UISlider alloc] init];
    self.progressBar.frame = CGRectMake(30, 0, frameWidth-30*2, 15);
...
//    [self.progressBar setThumbImage:[UIImage imageNamed:@"Slider_button"] forState:UIControlStateNormal];
    ...
    //Current Time Label时间
    self.playBackTime = [[UILabel alloc] init];
    [self.playBackTime sizeToFit];
    self.playBackTime.frame = CGRectMake(40, 0, 60, 30);
    ...
    self.zoomButton.frame = CGRectMake(frameWidth-30,0,image.size.width, image.size.height);
...
-(void)initLandscapeLayout{
    //横屏
    NSLog(@"横屏self.frame.size.width:%f    self.frame.size.height:%f",self.frame.size.width,self.frame.size.height);
    if (viewIsShowing) {//显示
    ...
    self.progressBar.frame = CGRectMake(40, 0, self.frame.size.width-80, 15);
    self.zoomButton.frame = CGRectMake(self.frame.size.width-30, 0, 20, 20);
}

-(void)initPortraitLayout{
    //竖屏
    NSLog(@"竖屏self.frame.size.width:%f    self.frame.size.height:%f",self.frame.size.width,self.frame.size.height);
    if (viewIsShowing) {
        self.playerHudBottom.frame = CGRectMake(0, self.frame.size.height-44, self.frame.size.width, 44);
    }else{
        self.playerHudBottom.frame = CGRectMake(0, self.frame.size.height, self.frame.size.width, 44);
